    ...
    def manage_open_positions(self, current_price):
        for position in self.open_positions[:]:  # 使用切片创建副本,以便在循环中安全地移除元素
            ...
            if position['type'] == "buy":
                ...
                new_take_profit = current_price + TAKE_PROFIT * self.atr
                should_close_sl = current_price <= new_stop_loss
                should_close_tp = current_price >= new_take_profit
            ...
            if should_close_sl or should_close_tp:
                close_reason = '止损' if should_close_sl else '止盈'
                self.log_and_update(
                    f"触发{close_reason}: 当前价格={current_price:.2f}, "
                    f"{'止损' if should_close_sl else '止盈'}价={new_stop_loss if should_close_sl else new_take_profit:.2f}")
                self.close_position(position)
                self.open_positions.remove(position)  # 平仓后从列表中移除该位置
            else:
                # 更新止损和止盈价格
                ...
                self.log_and_update(
                    ...
                    f"新止损价={position['stop_loss_price']:.2f}, 新止盈价={position['take_profit_price']:.2f}\n"
